...
class TextGenerationLoop
{
    ...
    protected function approvalAwareToolResults(array $toolCalls, array $tools, bool $isFinalStep = false): array
    {
        ...
        foreach ($toolCalls as $toolCall) {
            ...
            if ($isFinalStep || ! $tool instanceof Tool) {
                $exhausted[] = new ToolResult(
                    $toolCall->id,
                    $toolCall->name,
                    $toolCall->arguments,
        ...
        $toolResults = [...$exhausted, ...$this->executeToolCalls($runnable, $tools)];

        return [$toolResults, $pendingApprovals];
    }

    /**
     * Execute the given tool calls, batching adjacent concurrency-safe tools together.
     *
     * @param  ToolCall[]  $toolCalls
     * @param  Tool[]  $tools
     * @return ToolResult[]
    ...
    {
        if (count($batch) < 2) {
            return array_map(fn (array $pair): ToolResult => $this->executeToolCall(...$pair), $batch);
        }
        ...
        try {
            ...
            $tasks = array_map(
                static fn (array $pair): Closure => static fn (): array => static::handleSafely($pair[1], $pair[0]->arguments, $pair[0]->id),
                $batch,
            );
            ...
            try {
                ...
                $envelopes = array_map(fn (Closure $task): array => $task(), $tasks);
            }
    ...
    /**
     * Execute a single tool call and wrap it as a tool result.
     */
    protected function executeToolCall(ToolCall $toolCall, Tool $tool): ToolResult
    {
        return $this->toolResult($toolCall, $this->executeTool($tool, $toolCall->arguments, $toolCall->id));
    }
     ...
     */
    protected static function handleSafely(Tool $tool, array $arguments, ?string $toolCallId = null): array
    {
        try {
            return ['ok' => true, 'result' => (string) $tool->handle(new Request($arguments, $toolCallId))];
        } catch (Throwable $e) {
