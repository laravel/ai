...
class TextGenerationLoop
{
    ...
    /**
     * The run context of the generation currently executing on this loop, used by nested tool invocations.
     */
    protected ?RunContext $currentContext = null;
    ...
    public function generate(
        ...
        $restoreNestedToolRecords = $this->reserveNestedToolRecords($context);

        try {
            return $this->runGenerate(
                $provider, $model, $instructions, $messages, $tools, $schema,
                $options, $timeout, $approval, $recordApprovalResults, $context,
            );
        } finally {
    ...
    public function stream(
        ...
        try {
            yield from $this->runStream(
                ...
                $schema, $options, $timeout, $approval, $recordApprovalResults, $validatedApproval, $context,
            );
        } finally {
            $restoreNestedToolRecords();
        }
    ...
    public function invokeNestedTool(Tool $tool, string $name, array $arguments, string $toolCallId): string
    {
        ...
        try {
            $result = $this->executeTool($tool, $arguments, $toolCallId, $this->currentContext);
        } catch (Throwable $exception) {
     ...
     * @return Closure(): void
     */
    protected function reserveNestedToolRecords(?RunContext $context): Closure
    {
        [$calls, $results, $previousContext] = [$this->nestedToolCalls, $this->nestedToolResults, $this->currentContext];

        $this->nestedToolCalls = [];
        $this->nestedToolResults = [];
        $this->currentContext = $context;

        return function () use ($calls, $results, $previousContext): void {
