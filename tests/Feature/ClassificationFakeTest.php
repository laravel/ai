...
test('choice requires at least two options', function (): void {
    new Choice('Team?', ['billing' => null]);
})->throws(InvalidArgumentException::class, 'A choice question requires at least two options.');

test('boolean criteria may only describe true and false', function (): void {
    new Boolean('Did the build succeed?', ['maybe' => 'unclear']);
})->throws(InvalidArgumentException::class, 'Boolean criteria may only describe the "true" and "false" cases.');
...
test('fake generates shape-valid answers for every question', function (): void {
    ...
    expect($response['is_urgent'])->toBeInstanceOf(BooleanAnswer::class)
        ...
        ->and($response['department']->choice)->toBeIn(['billing', 'technical'])
...
test('fake answers may be overridden per question', function (): void {
    ...
    expect($response['department']->choice)->toBe('billing')
